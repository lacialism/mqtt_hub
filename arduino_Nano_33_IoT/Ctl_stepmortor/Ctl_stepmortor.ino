...
const char* dev_name = "nanoIoT_StepMortor";
...
// device setting
#define StepsPerRevolution 200  // 1 회전 당 스텝수를 의미
#define ROTATION 2              // 몇 번 회전할지를 결정 
#define RPM 60
#define IN1 8                   // IN1 은 아두이노 8번 핀에 연결을 의미
#define IN2 9
#define IN3 10
#define IN4 11
...
char func[2][CTL_MAX] = {"UP", "DOWN"}; // 수신받는 명령어
...
void control_callback(char* topic, byte* payload, unsigned int length){ // 메세지 받은 후 명령어 추출, 동작, 결과 보고 토픽 발행
  ...
  // ===== Code this block ===== //
  if(exist){
    if(funci == 0){ // Default: UP
      stepper.step(-StepsPerRevolution * ROTATION);
    }
    else if(funci == 1){  // Default: DOWN
      stepper.step(StepsPerRevolution * ROTATION);
    }
